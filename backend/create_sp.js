...
const sql = `
CREATE PROCEDURE sp_UpsertStudentApplication(
    IN p_student_id INT,
...
(async () => {
    try {
        ...
        await new Promise((resolve, reject) => {
            db.query("ALTER TABLE student_applications ADD COLUMN mobile_country_code VARCHAR(10) AFTER address_suburb", (err) => resolve());
        });
        await new Promise((resolve, reject) => {
            db.query("ALTER TABLE student_applications ADD COLUMN phone_country_code VARCHAR(10) AFTER contact1", (err) => resolve());
        });
        await new Promise((resolve, reject) => {
            db.query("ALTER TABLE student_applications ADD COLUMN education_data JSON AFTER relative_related_to", (err) => resolve());
        });

        // 2. Drop existing procedure if any
        await new Promise((resolve, reject) => {
            db.query("DROP PROCEDURE IF EXISTS sp_UpsertStudentApplication", (err) => {
                if (err) reject(err);
                else resolve();
            });
        });
        ...
        console.log('Procedure sp_UpsertStudentApplication created successfully');
        process.exit(0);
    } catch (err) {
